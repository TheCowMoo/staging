...
- client/src/components/AppLayout.tsx: added a new locked nav state that blocks navigation for **every** role (including ultra_admin), and set the 'Violent Incident Report Log' item (/violent-incident-log, which has no route/component) to locked: 'locked' (was coming-soon, which ultra admins bypassed into a dead link). Renders a lock icon + 'Locked' badge with a toast on click.