...
- `server/rasRouter.ts` + `client/src/pages/EmergencyAlerts.tsx`: sandbox users now **see** the RAS activation screen (Lockdown/Lockout buttons + standby panel + preview dialog) instead of the "Emergency Alerts Not Enabled" dead-end. Sandbox clicks on the buttons show a "locked for sandbox" toast (existing `isSandbox` handling). Sandbox users **cannot** be enrolled for push notifications: `savePushSubscription` switched to `paidSandboxRestrictedProcedure`, and the `PushEnrollmentBanner` is hidden when `isSandbox`. `activateAlert` + `getInstallerDownload` were already sandbox-restricted. Copy updated to state sandbox users won't receive emergency notifications.
- `client/src/pages/RASActivation.tsx`: `/ras/activate` (linked by the big red "RESPONSE ACTIVATION SYSTEM" buttons in `AppLayout`) now shows the 4 alert blocks (Lockdown/Lockout/Fire/Weather) for sandbox as a view-only preview instead of the "not enrolled" dead-end — clicks toast-block, installer download toast-blocks, facilityId is not required, and a "Sandbox mode — view only" notice was added. Non-sandbox users without a RAS role still see the original "Emergency Alerts Not Enabled" message.
- `client/src/pages/AdvisoryReport.tsx`: `/scan-report` (public route reached via the readiness scan's "Export Report" CTA) now gates the **admin-only** advisor content by role. "Save Advisor PDF" and the `.admin-only` "Discovery Call Notes — Private Advisor only" section are only available to platform advisors (`ultra_admin`/`admin`); everyone else (anonymous/client/sandbox/regular users) is forced into `client-mode` and can only export "Save Client PDF".

- `client/src/components/AppLayout.tsx`: the "Violent Incident Report Log" nav item (`/violent-incident-log`, no route/component) now shows "Coming Soon" for every role. The `coming-soon` lock no longer bypasses `ultra_admin` (removed `!isUltraAdmin`), so the item is locked with a "Soon" badge and a "coming soon" toast for all users, including platform admins.
- Incident reporting: added an "Anonymous communication" follow-up option (followUpMethod "in_app"). Reporters stay anonymous and message the safety team via the in-app tracking-token thread (CheckReport page) instead of sharing phone/email/in-person contact. Updated drizzle schema enum, incident submit zod enum, ReportIncident step 7 (3-option toggle + in-app info panel), IncidentDashboard display, and added a migrate entry (MODIFY incident_reports.followUpMethod add in_app).
- Migration SQL (apply on the server before deploy): ALTER TABLE incident_reports MODIFY COLUMN followUpMethod enum('phone','email','in_person','in_app') NULL;

### 2026-08-29 — California Violent Incident Log (SB 553 / Labor Code §6401.9)
- New tables: `violent_incident_logs` (PII-free by design — no victim/witness/perpetrator name, contact, address, SSN, or medical fields) and `violent_incident_log_requests` (15-day copy workflow with `notifiedDay1At`/`notifiedDay10At`/`notifiedDay14At` stamps). Both in `drizzle/schema.ts`; migration SQL: `drizzle/0046_violent_incident_log.sql`.
- New router `server/violentIncidentLogRouter.ts` (mounted as `violentIncidentLog` in `appRouter`): `create`/`list`/`get`/`listRequests`/`fulfillRequest` are admin-only (`orgAdminProcedure`) and org-scoped through the P0 authz chain (`requireOrgAccess` + `getUserOrgIds`). `requestLog`/`listMyRequests` are available to any authenticated employee. `requestLog` stamps the 15-calendar-day statutory deadline (`dueAt = requestedAt + 15d`) and notifies org admins (in-app + email). **No delete or update endpoints** — 5-year retention lock.
- New in-process scheduler `server/violentIncidentLogScheduler.ts`, started at boot from `server/_core/index.ts`: hourly `setInterval` fires Day 1 / Day 10 / Day 14 email + in-app reminders to org admins for pending requests. `checkDueRequests` is exported for tests and defensively skips non-pending requests.
