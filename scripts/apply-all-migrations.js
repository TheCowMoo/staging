 ...
 * Run this on the VPS: node scripts/apply-all-migrations.js
 */
...
async function main() {
  ...
  for (const file of files) {
    ...
    // Skip if already applied (check by filename in a tracking table)
    try {
  ...
  console.log("\nTables in database:");
  for (const name of ["training_modules", "notifications", "website_resource_links"]) {
    // Check actual table name variants
    const found = tableNames.find(t => t.includes(name.replace("_", "")) || t === name);
